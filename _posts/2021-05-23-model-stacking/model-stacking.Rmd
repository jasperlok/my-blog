...
- Drop 'ClaimNumber', 'num_week_paid_ult', 'InitialIncurredClaim Cost', 'UltimateIncurredClaimCost' & column headers that starts with 'acc' & 'report' since I am not using them later
...
- Sample a subset of the data to use for the analysis, otherwise it would take forever to run on my laptop `r emo::ji("cry")`
...
Before fitting a stack model, let's fit individual models first. To shorten the running time, I will skip the parameters tuning in this post.


Also as the focus of this post is on model stacking, hence I won't be going through the model building steps below. For more information on the model building steps, you can refer to my previous presentations on the [model building](https://jasperlok.netlify.app/posts/2021-04-03-sas-sharing-20210422/presentation/mitb_capstone_lok-jun-haur_slides#1) or the documentation page of [tidymodels](https://www.tidymodels.org/).



Nevertheless, let's start building the different machine learning models!
...
It's not the end of the world. With a few simple tricks, we would be able to get around this.
...
```{r, echo = FALSE, out.width = "135%"}
knitr::include_graphics("image/model stacking on predictions.png")
...
```{r, echo = FALSE, out.width = "70%"}
knitr::include_graphics("image/successful.jpg")
