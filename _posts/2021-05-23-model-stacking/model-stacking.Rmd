...
xgboost_spec <- 
  boost_tree() %>%
  #trees = 1000, min_n = tune(), tree_depth = 50, learn_rate = 0.01, 
...
To do so, I will use **stack** function to create an empty 
...
kknn_fit <- kknn_workflow %>%
  finalize_workflow(select_best(kknn_tune)) %>%
  last_fit(df_split)
